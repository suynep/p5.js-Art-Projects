...
class PerlinLine {
	...
	drawNoisePoints() {
		for (let i = 0; i <= width + 100; i += this.xStep) {
	...
	joinPoints(arr) {
		...
		for (let i = 1; i < arr.length; i++) {
			stroke(color((i + 210) % 360, 70, map(this.initY, 0, height, 30, 100)));
	...
	drawLineFinal() {
		...
		this.joinTwoConsecutive(this.pVecArr[this.iter - 1], this.pVecArr[this.iter]);
		// 
	...
	drawLineFinalReverse() {
		...
		this.drawNoisePoints();
		this.joinTwoConsecutive(this.pVecArr[this.iter], this.pVecArr[this.iter - 1]);
		// 
...
function setup() {
	createCanvas(400, numberOfLines * 10 + 50);
	frameRate(10);
	colorMode(HSB, 360, 100, 100);
	COLORS = [color(60, 20, 70)];
	time = frameCount;

	let incrVal = 0.05;
	for (let i = 0; i < numberOfLines; i++) {

		let niceLine = new PerlinLine(i / 2 * incrVal * 10, 20 + i * 1.5, Math.floor(numberOfLines - i / 1.7));
